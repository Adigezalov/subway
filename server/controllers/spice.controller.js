const Model = require('../models/spice.model')
...
module.exports.update = async function (req, res, next) {
	if (req.user.admin) {
		...
		try {
			...
			if (req.file) {
				if (updatable.image && updatable.image !== defaultImage.SPICE) {
				...
				if (updatable.image && !req.body.image) {
					fs.unlinkSync(`./${updatable.image}`)
					updated.image = defaultImage.SPICE
				}
			}
...
module.exports.delete = async function (req, res, next) {
	if (req.user.admin) {
		try {
			...
			if (deletedItem.image && deletedItem.image !== defaultImage.SPICE) {
