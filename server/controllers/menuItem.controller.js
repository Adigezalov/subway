const Model = require('../models/menuItem.model')
const DependentModel = require('../models/product.model')
...
module.exports.update = async function (req, res, next) {
	if (req.user.admin) {
		...
		try {
			...
			if (req.file) {
				if (updatable.image && updatable.image !== defaultImage.MENU_ITEM) {
				...
				if (updatable.image && !req.body.image) {
					fs.unlinkSync(`./${updatable.image}`)
					updated.image = defaultImage.MENU_ITEM
				}
			}
...
module.exports.delete = async function (req, res, next) {
	if (req.user.admin) {
		try {
			const dependentItems = await DependentModel.find({menuItem: req.params.id})

			if (dependentItems.length) {
				helpers.clientErrorHandler(res, 400, errorText.ERROR_8)
			} else {
				const deletedItem = await Model.findById(req.params.id)

				if (deletedItem.image && deletedItem.image !== defaultImage.MENU_ITEM) {
