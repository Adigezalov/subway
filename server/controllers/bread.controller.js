const Model = require('../models/bread.model')
...
module.exports.update = async function (req, res, next) {
	if (req.user.admin) {
		...
		try {
			...
			if (req.file) {
				if (updatable.image && updatable.image !== defaultImage.BREAD) {
				...
				if (updatable.image && !req.body.image) {
					fs.unlinkSync(`./${updatable.image}`)
					updated.image = defaultImage.BREAD
				}
			}
...
module.exports.delete = async function (req, res, next) {
	if (req.user.admin) {
		try {
			...
			if (deletedItem.image && deletedItem.image !== defaultImage.BREAD) {
