const Model = require('../models/assemblyDiagram.model')
...
module.exports.update = async function (req, res, next) {
	if (req.user.admin) {
		...
		try {
			...
			if (req.file) {
				if (updatable.image && updatable.image !== defaultImage.ASSEMBLY_DIAGRAM) {
				...
				if (updatable.image && !req.body.image) {
					fs.unlinkSync(`./${updatable.image}`)
					updated.image = defaultImage.ASSEMBLY_DIAGRAM
				}
			}
...
module.exports.delete = async function (req, res, next) {
	if (req.user.admin) {
		try {
			...
			if (deletedItem.image && deletedItem.image !== defaultImage.ASSEMBLY_DIAGRAM) {
