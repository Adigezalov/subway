...
export function fetchPromotionAction({id}) {
	return async (dispatch, getStat) => {
		try {
			dispatch(showLoaderAction())
			const response = await axios({
...
export function createPromotionAction({editItem}) {
	...
	return async (dispatch, getStat) => {
		try {
			...
			const response = await axios({
				...
				url: editItem._id ? `${API_URL}/api/promotion/${editItem._id}` : `${API_URL}/api/promotion`,
				data: data,
				headers: {
...
export function deletePromotionAction({id}) {
	return async (dispatch, getStat) => {
		try {
			dispatch(showLoaderAction())
			const response = await axios({
...
export function cleanPromotionAction() {
	return {
		type: types.CLEAN,
	}
}
