...
export function fetchSauceAction({id}) {
	return async (dispatch, getStat) => {
		try {
			dispatch(showLoaderAction())
			const response = await axios({
				method: 'GET',
				url: `${API_URL}/api/sauce/${id}`,
				headers: {
					Authorization: getStat().app.token,
				},
			})
...
export function createSauceAction({editItem}) {
	let data = new FormData()
	data.append('name', editItem.name)
	data.append('isSweet', editItem.isSweet)
	...
	return async (dispatch, getStat) => {
		try {
			...
			const response = await axios({
				...
				url: editItem._id ? `${API_URL}/api/sauce/${editItem._id}` : `${API_URL}/api/sauce`,
				data: data,
				headers: {
...
export function deleteSauceAction({id}) {
	return async (dispatch, getStat) => {
		try {
			dispatch(showLoaderAction())
			const response = await axios({
...
export function cleanSauceAction() {
	return {
		type: types.CLEAN,
	}
}
